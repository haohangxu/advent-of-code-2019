...
module Intcode = struct
  ...
  let interpret (state : int Array.t) inputs =
    let rec interpret_next instr_num inputs outputs =
      ...
      in
      let do_input () =
        state.(state.(instr_num + 1)) <- (List.hd_exn inputs);
        interpret_next (instr_num + 2) (List.tl_exn inputs) outputs
      in
      let do_output () =
        interpret_next (instr_num + 2) inputs (state.(state.(instr_num + 1)) :: outputs)
      in
...
module Amplifier = struct
  let all_permutations =
    ...
    in
    permutations (List.init 5 ~f:Fn.id)
  ;;
  ...
  let run state phases =
    ...
    List.fold phases ~init:0 ~f:(fun signal phase -> 
        ...
        | [ signal ] -> signal
        | _ -> raise_s [%message "Unexpected number of outputs!"])
  ;;
end

module Part1 = struct
  let find_best_phases state =
    List.map ~f:(Amplifier.run state) Amplifier.all_permutations
    ...
    Command.basic
      ...
      (Command.Param.map filename_param ~f:(fun filename ->
           (fun () ->
             ...
             assert (List.length input = 1);
             let initial_state = List.hd_exn input |> Intcode.state_of_string in
             let (_ : int list) = Intcode.interpret initial_state [ 5 ]  in
             ())))
  ;;
end
